...
def join_columns(row):
    query_origin = '-'.join([row['type_origin'], row['gem_name']]).lower().replace('superior-', '').replace("'", '').replace(' ', '-') + '-20-20'
    query_target = '-'.join([row['type_target'], row['gem_name']]).lower().replace('superior-', '').replace("'", '').replace(' ', '-') + '-20-20'
...
# 保存df_merged
df_merged.to_csv('POE/alt_gem/gem_analysis_lvl20_{}.csv'.format(date_str), index=False)
